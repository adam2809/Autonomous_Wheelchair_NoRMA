...
from base_driver import BaseJoystickDriver
...
class GC2JoystickDriver(BaseJoystickDriver):
    def __init__(self):
        self.MAX = 2047
        self.FB_CENTER = 1233
        self.LR_CENTER = 1232
        ...
        existing_calib = check_for_existing_calibration()
        if any(x == -1 for x in existing_calib)
            rospy.loginfo("No calibration values provided. Running calibration now...")
            new_calib = self.sample_new_calib_vals()
            rospy.loginfo(f"New calibration values fb: {new_calib[0]} lr: {new_calib[1]}")
            self.joystick.set_calibration_values(*new_calib)
        else:
            self.joystick.set_calibration_values(*existing_calib)
            rospy.loginfo(f"Calibration values loaded fb: {existing_calib[0]} lr: {existing_calib[1]}")


    def check_for_existing_calibration():
        try:
    ...
    def sample_new_calib_vals(self, samples = 1024):
