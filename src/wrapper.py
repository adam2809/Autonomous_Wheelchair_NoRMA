...
class Wheelchair_virtual_joystick_driver:
    def __init__(self):
        if is_test:
            from joystick_driver.gazebo_driver import GazeboJoystickDriver
            self.joystick = GazeboJoystickDriver()
        else:
            from joystick_driver.gc2_driver import GC2JoystickDriver
            self.joystick = GC2JoystickDriver()
